...
def test_se_connecter_oui():
    ...
    # WHEN
    with patch("dao.utilisateur_dao.UtilisateurDao.trouver_par_pseudo", return_value=liste_utilisateurs[1]):
        with patch("dao.utilisateur_dao.UtilisateurDao.se_connecter", return_value=True):
...
def test_changer_mdp_identique():
    ...
    utilisateur = Utilisateur(
        pseudo="jp",
        mdp=hash_password("old", "Français"),
        age=20,
        langue="Français"
    )
    service = UtilisateurService()
